...
describe('compareWithNull', () => {
  const array = [3, null, 1, NaN, 2, NaN, 1, null, 0];

  test('sort with null by ascending', () => {
    expect(array.sort(compareWithNull)).toEqual([0, 1, 1, 2, 3, null, null, NaN, NaN]);
  });

  test('sort with null by descending', () => {
    expect(array.sort((a, b) => compareWithNull(a, b, 'descending'))).toEqual([3, 2, 1, 1, 0, null, null, NaN, NaN]);
  });
